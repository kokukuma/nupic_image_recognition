...
def createMyNetworkSimple():
    ...
    # set SP
    PARAMS['SP']["inputDimensions"]             = (50, 4, 4)
    PARAMS['SP']["columnDimensions"]            = (25, 8, 8)
...
def createMyNetworkComplex():
    ...
    # set SP
    PARAMS['SP']["inputDimensions"]             = (5, 24, 24)
    PARAMS['SP']["columnDimensions"]            = (5, 8, 8)
    PARAMS['SP']["numActiveColumnsPerInhArea"]  = int(15. * 8. * 8. * 0.02)
    network.addRegion("simplePoolerRegion", "SP", PARAMS['SP'])

    # # set TP
    # PARAMS['TP']['numberOfCols']   = 5 * 8 * 8            # default: 64 * 64
    ...
    network.link("simplePoolerRegion", "classifierRegion")
    ...
    for i, data in enumerate(train_data[datanum:datanum+length]):
        ...
        if i>= 0:
            if base_sdr == None:
                ...
                if len(set(base_sdr) & set(activeColumns.nonzero()[0]))  >= len(base_sdr)/1.5:
                    #print datanum_i, label_name, i, activeColumns.nonzero()[0]
...
if __name__ == "__main__":
    ...
    for i in range(40):
        ...
        simpleNetwork, simple2Network, simple3Network = \
            runCifar10Network(train_data, train_label, simpleNetwork, simple2Network, simple3Network, complexNetwork,datanum, length=500, learnMode=[True, True, True, True])
